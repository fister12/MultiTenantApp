...
export default function EditNotePage() {
  ...
  const fetchNote = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (noteData.userId !== user?.userId && user?.role !== 'ADMIN') {
          throw new Error('You do not have permission to edit this note');
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm() || !token || !noteId) return;
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-16">
              <div className="flex items-center space-x-4">
                <Link href={`/notes/${noteId}`} className="text-blue-600 hover:text-blue-800">
                  ← Back to Note
                </Link>
            ...
            {note && !loading && (
              <div className="bg-white shadow rounded-lg">
                <div className="px-6 py-4 border-b border-gray-200">
                  <h2 className="text-lg font-medium text-gray-900">Edit Note</h2>
